import { runReadmeGeneration } from "./src/lib/readme.functions.ts";
...
async function main() {
  ...
  for (const kw of FORBIDDEN) {
    if (new RegExp("\\b" + kw.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "\\b", "i").test(r.readme) &&
        !(kw === "Go programming" && false)) {
      // Allow the word "Go" only as a verb, "go get", etc. We check specific phrases below.
  ...
  // language-aware check: "Go" as a standalone word could be a verb ("go build").
  if (/\bGo\b/.test(r.readme) && !/\bGo (build|run|mod|get|test)\b/i.test(r.readme) && !/\bGoLang\b/i.test(r.readme) && !/\bGolang\b/i.test(r.readme)) {
