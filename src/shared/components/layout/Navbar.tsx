...
import { IconLogOut as LogOut } from "@/shared/components/icons";
...
type NavTarget = "/" | "/generate" | "/graph";

function navTarget(link: NavLink): NavTarget {
  return link.to === "/#how" ? "/" : link.to;
}
...
export function Navbar() {
  ...
  const handleNavClick = (l: NavLink) => {
    setMenuOpen(false);
    if (l.hash) {
  ...
  return (
    <header className="site-navbar sticky top-0 z-50 bg-cream/80 backdrop-blur-xl border-b border-bone/60">
      <div className="relative mx-auto flex h-16 min-w-0 max-w-[1200px] items-center justify-between gap-3 px-4 sm:h-20 sm:px-6 lg:px-8">
        ...
        <nav className="navbar-pill absolute left-1/2 hidden w-fit -translate-x-1/2 items-center justify-center gap-1 rounded-[14px] border p-1.5 md:flex">
          {NAV_LINKS.map((l) => (
            <Link
              ...
              className={[
                "navbar-link relative flex items-center gap-1.5 rounded-[10px] px-3 py-2 text-sm no-underline",
                "transition-all duration-200 outline-none [&>svg]:w-[15px]",
                "[&:not(:active):not(.active-link):hover]:bg-graphite/15",
                "[&:focus:not(.active-link)]:bg-graphite/15",
                "focus:bg-graphite/20 active:bg-graphite/20 [&.active-link]:bg-graphite/20",
                l.muted ? "opacity-[0.78] hover:opacity-100 focus:opacity-100" : "",
              ].join(" ")}
              activeProps={l.hash ? undefined : { className: "active-link" }}
            >
              {l.icon}
              {l.label}
            </Link>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3.5">
          {user ? (
            <>
              ...
              <button
                ...
                className="hidden sm:inline-flex items-center gap-1.5 h-10 px-4 rounded-full border border-bone/80 bg-paper/30 text-xs text-ink/70 shadow-subtle hover:-translate-y-0.5 hover:text-ink hover:border-ink/30 transition-[background-color,border-color,color,transform] duration-300"
                aria-label="Sign out"
              >
                <LogOut size={12} />
                <span>Sign out</span>
          ...
          <button
            ...
            className="md:hidden w-10 h-10 rounded-full border border-bone/80 bg-paper/30 flex flex-col items-center justify-center gap-1.5 shadow-subtle hover:bg-paper transition-colors"
          ...
          <div className="navbar-theme-toggle order-last ml-auto sm:ml-2">
            <ThemeToggle />
          </div>
        </div>
      </div>
      ...
      {menuOpen && (
        <nav className="md:hidden border-t border-bone/60 bg-cream/95 backdrop-blur-xl rounded-b-3xl shadow-md">
          <div className="mx-auto max-w-[1200px] px-4 sm:px-6 py-4 flex flex-col gap-1.5">
            {NAV_LINKS.map((l) => (
              <Link
                key={l.label}
                to={navTarget(l)}
                onClick={() => handleNavClick(l)}
                className={`px-4 py-3 text-base rounded-2xl transition-colors flex items-center gap-3 ${l.muted ? "text-ink/70 hover:text-ink hover:bg-paper" : "text-ink/90 hover:text-ink hover:bg-paper"}`}
              >
            ...
            {user && (
              <button
                ...
                className="flex items-center gap-3 px-4 py-3 text-base rounded-2xl text-ink/80 hover:text-ink hover:bg-paper transition-colors"
              >
                <LogOut size={15} />
                Sign out
              </button>
